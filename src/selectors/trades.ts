import {
  ...
  joinTargetsToPositions,
} from '../selectors/display';
...
function flatten_targets(
  ...
  return Object.values(
    targets
      .flatMap<FlattenedTargetPosition>((t) => {
        ...
        const positions_lookup = Object.fromEntries(
          t.positions.map((p) => [p.isin, p])
        );
        ...
        return direct_targets.map(({ isin, ticker, weight }) => {
 ...
 * other things.
 */
function find_target_amounts(flattened_targets: FlattenedTargetPosition[]) {
  let start_total = 0;
  let target_total = 0;
...
function filter_targets(
  ...
  const cleansed = adjusted_targets
    ...
    .filter(
      ({ instruments, weight }) => instruments.length && weight > min_weight
    );
...
function make_buy_trades(
  ...
  const catchup_liquid = Math.max(
    0,
    Math.min(target_total - start_total, total_liquid)
  );
  const normal_liquid = total_liquid - catchup_liquid;

  const action = ACTION_TYPE.BUY;
  return targets
    .map<Trade>(({ instruments, weight, value }) => {
      ...
      return {
        ...
        value: isNaN(price)
          ? -total_value
          : -price * Math.floor(total_value / price),
      };
    })
...
export const selectTradesWhereConfigs = createSelector(
  ...
  (
    ...
    const total =
      starting_positions.reduce((sum, { value }) => sum + value, 0) + cash;
    const sell_trades = make_sell_trades(
      starting_positions,
      prices,
      sell_config
    );
