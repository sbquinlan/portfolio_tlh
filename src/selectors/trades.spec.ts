...
const TARGETS = [
  fake_aggregation(fake_target(['VTI', 'SCHB'], 0.35), [
    fake_position('VTIB', 1000, 1000, 1000, 0, 0),
    fake_position('SCHB', 1000, 0, 0, -1000, 1000),
  ]),
  fake_aggregation(fake_target(['TWIB', 'MUB'], 0.35), []),
  fake_aggregation(fake_target(['VEA', 'SCHF'], 0.1), [
    fake_position('VEA', 1000, 0, 0, -1000, 1000),
  ]),
  fake_aggregation(fake_target(['VWO', 'IEMG'], 0.1), [
    fake_position('IEMG', 1000, 100, 1000, 0, 0),
  ]),
  fake_aggregation(fake_target(['SCHD', 'VIG'], 0.1), [
...
describe('get_adjusted_portfolio', () => {
  it('calculates an updated portfolio based on sales', () => {
    ...
    const total_liquid = orders.reduce((sum, o) => sum + o.value, 0);
    const initial_total = TARGETS.reduce(
      (t_sum, t) =>
        t_sum + t.positions.reduce<number>((p_sum, p) => p_sum + p.value, 0),
      0
    );

    // the sum of all adjusted values
    const adj_total = adj_values.reduce((sum, [_, amt]) => sum + amt, 0);
    // find the max ratio between target weight and current weight
    const max_total = adj_values
