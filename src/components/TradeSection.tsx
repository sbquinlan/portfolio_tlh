...
function TradeSection({ }: TProps) {
  ...
  const wash_sale_options = useMemo(
    () => rank_options(wash_sale_search.toUpperCase(), all_tickers, wash_sale, t => t),
  ...
  const offset_gains_options = useMemo(
    () => rank_options(offset_gains_search.toUpperCase(), all_tickers, offset_gains, t => t),
  ...
  const buy_orders = adjusted_values
    ...
    .map<Trade>(
      ([t, p]) => ({
        ...
        gain: 0, 
        loss: 0,
        value: -1 * (Math.min(catchup_amount, total_liquid) * p + 
