...
const TickerTokenList = React.forwardRef<
  HTMLButtonElement | null,
  TTokenListProps<string>
      ...
      {elements.map((elm, i) => {
        return (
          <button
            ...
            role="button"
            onKeyDown={(e) => {
              if (e.key === 'Backspace') onRemove(elm);
            }}
          >
            <span>{elm}</span>
            <Xmark
              className="inline-block align-middle h-4 w-4"
              onClick={() => onRemove(elm)}
            />
          </button>
        );
      })}
