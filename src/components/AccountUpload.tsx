  ...
  return new Promise<any[]>((res, rej) => {
    Papa.parse(
      ...
      { 
        skipEmptyLines: true, 
        header: true,
        complete(r, _) { res(r.data); },
        error(e: any, _: any) { rej(e); }
      },
    );
  });
}

export default function AccountUpload({ onChange }: { onChange: (p: Map<string, AccountPosition>) => void}) {
  ...
  return (
    <input className='block text-sm file:mr-4 file:py-2 file:px-4 file:rounded-sm file:border-0 file:text-sm file:font-semibold file:bg-gray-300 hover:file:bg-gray-200' type="file" accept='csv' onChange={_onChange} />
  )
}