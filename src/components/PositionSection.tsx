...
import { IKeyableRow, CustomColumn, TableColumn, ValueColumn } from "./SortableTable";
...
export class MoneyColumn<TRow extends IKeyableRow> extends ValueColumn<TRow, number> {
  constructor(
    header: string,
  ...
  renderFooter(rs: TRow[], props?: React.TdHTMLAttributes<HTMLTableCellElement>) {
    return (
...
function PositionSection({ positions, setPositions }: TProps) {
  const cols = useMemo(
    () => [
      ...
      new MoneyColumn<DisplayTargetState>('Value', r => r.value, '', 'text-center text-sm w-24'),
      new MoneyColumn<DisplayTargetState>('Target', _ => 0, '', 'text-center text-sm w-24'),
      new MoneyColumn<DisplayTargetState>('Uplots', r => r.uplots, '', 'text-center text-sm w-24'),
      new MoneyColumn<DisplayTargetState>('Downlots', r => r.downlots, '', 'text-center text-sm w-24'),
      new MoneyColumn<DisplayTargetState>('Net', r => r.uplots + r.downlots, '', 'text-center text-sm w-24'),
    ],
    []
  );
  const nestedCols = useMemo(
    () => [
      ...
      new MoneyColumn<AccountPosition>('Value', r => r.value, '', 'text-center text-xs w-24'),
      // placeholder for "target" which doesn't exist here
      new CustomColumn(() => <th />, () => (<td className="text-center text-xs w-24">--</td>)),
      new MoneyColumn<AccountPosition>('Uplots', r => r.uplots, '', 'text-center text-xs w-24'),
      new MoneyColumn<AccountPosition>('Downlots', r => r.downlots, '', 'text-center text-xs w-24'),
      new MoneyColumn<AccountPosition>('Net', r => r.uplots + r.downlots, '', 'text-center text-xs w-24'),
    ],
    []
  );
  return (
        ...
        footer={
          <tfoot>
            <tr className="border-t-2">
              <td />
              {cols.map(c => c.renderFooter(positions, { className: "text-center text-sm font-semibold" }))}
            </tr>
          </tfoot>
        }
      />
