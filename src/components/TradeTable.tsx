...
import { MoneyColumn, NumberColumn, SortableTable, StringColumn, TSortableTableChildProps } from "../ui/SortableTable";
...
function TradeTableBody({
  rows,
  cols,
}: TSortableTableChildProps<Trade>) {
  return (
    <tbody>
      {rows.map(r => (
        <tr key={r.key} className={r.order === 'buy' ? 'bg-green-200' : 'bg-red-200'}>
          {cols.map(c => (
      ...
      <tr className="bg-gray-200">
        {cols.map(c => (
          <td key={c.key} className='text-center text-sm font-semibold'>
            {c instanceof NumberColumn ? format_dollas(rows.reduce((sum, row) => sum + c.getValue(row), 0)) : '--'}
          </td>
        ))}
      </tr>
    </tfoot>
