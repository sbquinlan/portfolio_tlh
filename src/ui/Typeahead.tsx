...
export function Typeahead<TDataElement>({
  ...
  const [{ highlight, open }, setTrayState] = useState<{
    open: boolean;
    highlight: number;
  }>({
    highlight: 0,
  ...
  const _onFocus = link_event_handlers(onFocus, () => {
    setTrayState((_) => ({ highlight: 0, open: options.length > 0 }));
  });
  const _onBlur = link_event_handlers(onBlur, () => {
    setTrayState((_) => ({ highlight: 0, open: false }));
  });
  const _onKeydown = link_event_handlers(onKeyDown, (e) => {
    switch (e.key) {
      case 'Escape':
        setTrayState((_) => ({ highlight: 0, open: false }));
        break;
      ...
      case 'ArrowUp':
        setTrayState(({ highlight, open }) => ({
          highlight: open
            ? (highlight + options.length - 1) % options.length
            : highlight,
          open,
        }));
        break;
