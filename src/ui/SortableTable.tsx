...
export class TableColumn<TRow extends IKeyable, TValue extends ReactNode>
  implements IKeyable
{
  constructor(
...
export class NumberColumn<TRow extends IKeyable> extends SortableColumn<
  TRow,
  number
> {
  constructor(label: string, getValue: (r: TRow) => number) {
    super(label, getValue, (a: TRow, b: TRow) => getValue(b) - getValue(a));
  }
}
...
export class StringColumn<TRow extends IKeyable> extends SortableColumn<
  TRow,
  string
> {
  constructor(label: string, getValue: (r: TRow) => string) {
    super(label, getValue, (a: TRow, b: TRow) =>
...
export type SortInfo<TRow extends IKeyable> = [
  1 | -1,
  SortableColumn<TRow, any> | undefined
];
...
export function SortableColumnHeader<TRow extends IKeyable>({
  ...
  if (!(col instanceof SortableColumn)) {
    return (
      <th {...props} key={col.key}>
        {col.label}
      </th>
    );
  }
  const sort_handler = () => {
    setSort([direction === 1 ? -1 : 1, col]);
  };
  return (
...
export function SortableTable<TRow extends IKeyable>({
  ...
  return (
    <table className="w-full table-auto" {...rest}>
      <SortableTableHeader
        className="bg-gray-200 cursor-pointer text-xs"
        cols={cols}
        sort={sort}
        setSort={setSort}
      />
