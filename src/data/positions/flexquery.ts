...
async function _xmlToJSON<TPayload extends Record<string, string | null>>(resp: Response): Promise<TPayload> {
  const xml = new DOMParser().parseFromString(await resp.text(), 'text/xml')
  return Object.fromEntries(
    Array.from(xml.querySelectorAll(':root > *')).map(elm => [elm.tagName, elm.textContent])
  ) as TPayload;
}
...
async function _fetchStatement(
  ...
  if (attempts > 3) {
    throw new Error(ERROR_CODES[payload.ErrorCode as keyof typeof ERROR_CODES] || payload.ErrorMessage as string);
  }
...
export async function fileFromFlexQuery({ token, query_id }: FlexQueryParams): Promise<Blob> {
  const resp = await fetch(`/flex/Universal/servlet/FlexStatementService.SendRequest?t=${token}&q=${query_id}&v=3`);
  if (!resp.ok) {
    throw new Error(await resp.text());
  }
  ...
  return await _fetchStatement({ token, reference_code: payload.ReferenceCode! });
}