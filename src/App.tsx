...
function App() {
  const [targets, setTargets] = useState([
    ...
    new TargetSimplePosition('VCITX', 'Vanguard CA Muni', 0.30),
    new TargetMultiPosition(['VIG', 'SCHD'], 0.10), // dividend
    new TargetMultiPosition(['VEA', 'SCHF'], 0.10), // international
    new TargetMultiPosition(['IEMG', 'VWO'], 0.10), // emerging
    
    new TargetMultiPosition(['VBK', 'VB'], 0), // small cap
    new TargetSimplePosition('VOT', 'Vanguard Mid-Cap Growth Index Fund ETF', 0),

    new TargetSimplePosition('F', 'Ford', 0),
    new TargetSimplePosition('AAL', 'American Airlines', 0),
  ])
  const [positions, setPositions] = useState<Map<string, AccountPosition>>(new Map());
    ...
    (acc: DisplayTargetState[], target: TargetPosition) => {
      if (target instanceof TargetMultiPosition) {
        ...
        return acc.concat(
          new DisplayTargetState(
            target,
            positions.has(target.ticker)
              ? [positions.get(target.ticker)!]
              : [],
          )
        );
      }
  ...
  return (
    <div className='container mx-auto px-10'>
      <TargetSection 
        targets={targets} 
        setWeight={(t, w) => { t.weight = w }} 
        onDelete={(t) => { setTargets(targets.filter(v => t !== v)) }}
      />
      <PositionSection 
